...
PlayerState_WeaponOut_MoveLeft::PlayerState_WeaponOut_MoveLeft()
{
	m_animSpeed = 0.35f;
}
...
PLAYER_STATE PlayerState_WeaponOut_MoveLeft::Update(Player* _player, float _deltaTime)
{
	// バインドしたプレイヤーから状態の取得
...
void PlayerState_WeaponOut_MoveLeft::EnterState(Player* _player)
{
	...
	if (_player->GetToggleWalk())
	{
		...
		// 歩行状態の時
		skel->PlayAnimation(_player->GetAnim(PLAYER_STATE::STATE_WEAPONOUT_WALKLEFT), m_animSpeed);
	}
	else
	{
		m_animSpeed = 0.35f;
		...
		// 通常走り状態
		skel->PlayAnimation(_player->GetAnim(PLAYER_STATE::STATE_WEAPONOUT_MOVELEFT), m_animSpeed);
	}
}

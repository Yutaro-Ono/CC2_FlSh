...
public:
	...
	virtual bool Load(const std::string& in_fileName) = 0;       // メッシュのロード
	void Delete();                                                                // メッシュの解放

	// テクスチャの追加 (メッシュファイルと同階層のディフューズ・スペキュラ・ノーマルを取得)
	void AddTexture(const std::string& in_meshName);
	...
	int CreateTextureStage(TEXTURE_TYPE in_type, const std::string& in_filePath);
